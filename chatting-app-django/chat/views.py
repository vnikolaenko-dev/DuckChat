...
def message_view(request, sender, receiver):
    ...
    if request.method == "GET":
        ...
        list1 = [(aes.dec_aes(i.message, rsa.decript(secret_key1.aes_key, secret_key1.secret_key)), 1, i.timestamp.strftime("%m-%d %H.%M")) for i in Message.objects.filter(sender_id=sender, receiver_id=receiver)]
        list2 = [(aes.dec_aes(i.message, rsa.decript(secret_key2.aes_key, secret_key2.secret_key)), 0, i.timestamp.strftime("%m-%d %H.%M")) for i in Message.objects.filter(sender_id=receiver, receiver_id=sender)]
